...
const PROBE_PREFIX = `__probe_${process.pid}_${Date.now()}`
...
const probes: Record<string, string> = {
  [path.join(SRC_PATH, `${PROBE_PREFIX}_barrel__.ts`)]: [
  ...
  [path.join(SRC_PATH, `${PROBE_PREFIX}_removed_barrel_icon__.ts`)]: [
    `import {AccessDeniedIcon} from './index'`,
    `console.log(AccessDeniedIcon)`,
    ``,
  ].join('\n'),
  [path.join(SRC_PATH, `${PROBE_PREFIX}_subpath__.ts`)]: [
